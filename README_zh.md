...
本项目在官方 `Fun-ASR-Nano-2512` 基础模型上完成了上海话语音识别方向的本地数据准备和 LoRA 微调。基础模型未上传到本仓库，运行时可从 ModelScope 或 Hugging Face 下载。

### 已完成的工作

- 整理上海话对话语音数据，并生成 Fun-ASR 训练所需的 `wav.scp`、文本标注和 JSONL 文件。
- 使用约 13.94 小时上海话语音数据进行微调；对音频片段进行时长过滤，保留 0.3 至 30 秒的片段，共写入 12,484 条片段，过滤 182 条。
- 按固定随机种子 `20260728` 划分数据集：训练集 10,741 条，验证集 1120 条，测试集 1323 条。
- 新增 `train/model_fit.py`，用于加载本地基础模型并启动 CUDA 混合精度 LoRA 微调。
- 训练输出和 TensorBoard 日志保存在本地 `output_shanghai2/` 目录，用于后续评估和恢复训练。

### 与官方 Fun-ASR 的差异

本项目没有修改 Fun-ASR 的核心模型结构、推理流程或官方模型权重；主要变化是加入了本地上海话数据、数据划分文件和微调启动脚本。根目录中的 `data/` 文件反映本地数据准备结果，不再是官方示例数据。

### 训练结果说明

最终训练 checkpoint 位于本地 `output_shanghai2/model.pt/`，其中包含模型参数和优化器状态。由于 checkpoint 体积较大，模型权重、训练输出、日志、音频和压缩包均不会提交到 Git；本仓库只保存可复现训练所需的代码、配置和数据清单。
